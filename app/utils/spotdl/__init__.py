...
from app.utils.spotdl.utils.search import parse_query
from app.utils.spotdl.utils.spotify import SpotifyClient

__all__ = ["Spotdl", "console_entry_point", "__version__"]

# Lazy import of console_entry_point to avoid import errors when console is not needed
try:
    from app.utils.spotdl.console import console_entry_point
except ImportError:
    # Console module not available or has dependencies issues
    # Define a no-op function
    def console_entry_point():
        raise RuntimeError("Console entry point is not available. This is expected when using spotDL as a library.")
